...
//#import "UIImage.h"
#import "UIImage-UIImagePrivate.h"
...
- (UIImage*)screenShot
{
    ...
    uint32_t width = IOSurfaceGetWidth(_surface);
    uint32_t height = IOSurfaceGetHeight(_surface);
    
    
    
    CGDataProviderRef provider = CGDataProviderCreateWithData(NULL, IOSurfaceGetBaseAddress(_surface), (width * height * 4), NULL);
    CGImageRef cgImage=CGImageCreate(width, height, 8,
                                     8*4, IOSurfaceGetBytesPerRow(_surface),
...
//    return;
    ...
    IOSurfaceAcceleratorTransferSurface(outAcc, _surface, destSurf, dict, NULL);
    
    uint32_t aseed;
    IOSurfaceUnlock(_surface, kIOSurfaceLockReadOnly, &aseed);
    CFRelease(outAcc);
